      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="bg-gradient-to-r from-red-600 to-pink-600 rounded-[2.5rem] shadow-xl p-8 mb-8 text-white relative overflow-hidden">
          <div className="flex items-center justify-between relative z-10">
            <div>
              ...
              <p className="text-red-100 font-bold uppercase text-[10px] tracking-widest opacity-80">Kampüs Hiyerarşisi ve Sistem Yönetimi</p>
            </div>
            <div className="text-right">
              <div className="text-4xl font-black">{stats?.users || 0}</div>
              <div className="text-[10px] font-black uppercase tracking-widest text-red-100">Toplam Kayıtlı Öğrenci</div>
            </div>
          </div>
            ...
            return (
              <button
                ...
                className={`flex items-center space-x-2 px-6 py-3 rounded-xl font-black uppercase text-xs tracking-widest transition-all ${
                  activeTab === tabItem.id ? 'bg-red-600 text-white shadow-lg' : 'text-gray-400 hover:bg-gray-50'
                }`}
              >
                <Icon size={16} />
        ...
        <div className="bg-white rounded-[2rem] shadow-sm p-8 border border-gray-100">
          <AnimatePresence mode="wait">
            ...
            >
              {activeTab === 'overview' && <OverviewTab stats={stats} onCardClick={changeTab} />}
              {activeTab === 'users' && (
                <UsersTab
                  users={users}
                  loading={loading}
              ...
              <div className="p-8 -mt-12 text-center">
                ...
                <div className="grid grid-cols-1 gap-3">
                  ...
                  {selectedUser.role !== 'club_admin' ? (
                    ...
                    >
                      <Crown size={18} /> Başkan Yetkisi Ver
                    </button>
                  ) : (
                    <button 
          ...
          return (
            ...
            >
              ...
              <div className="text-[9px] text-indigo-500 font-bold uppercase opacity-0 group-hover:opacity-100 transition-opacity flex items-center">
                Detaylara Git <ChevronRight size={10} className="ml-1" />
              </div>
            </motion.div>
          );
        })}
    ...
    <div>
      <div className="flex flex-col md:flex-row md:items-center justify-between mb-8 gap-4 text-left">
        ...
        <div className="flex items-center bg-gray-50 px-5 py-3 rounded-2xl border-2 border-gray-100 focus-within:border-red-200 transition-all">
          ...
          <input
            type="text"
            placeholder="Öğrenci veya Email Ara..."
      ...
      {loading ? (
        <div className="text-center py-20 text-gray-300 font-black uppercase text-xs animate-pulse italic">Kullanıcılar Senkronize Ediliyor...</div>
      ) : (
        <div className="grid grid-cols-1 gap-3">
          {users.map(user => (
            <div 
              ...
              >
                <div className="flex items-center space-x-5">
                  ...
                  <div className="text-left">
                    ...
                    <div className="text-[10px] text-yellow-600 font-black tracking-widest uppercase">Yeni Başvuru Alındı</div>
                  </div>
                </div>
            ...
            >
              <div className="flex items-center space-x-4">
                ...
                <div className="text-left">
                  ...
                  <div className="text-[9px] text-emerald-600 font-black uppercase tracking-widest">Sistemde Kayıtlı</div>
                </div>
              </div>
              <button
                onClick={(e) => { e.stopPropagation(); handleDeleteClub(club.id); }}
...
function AnnounceTab() {
  ...
  const handleSend = async () => {
    if (!message.trim()) { toast.warning('Lütfen bir mesaj yazın!'); return; }
    if (!window.confirm('TÜM kullanıcılara gönderilecek. Emin misiniz?')) return;
    setSending(true);
    try {
    ...
    <div className="text-left">
      ...
      <div className="bg-gradient-to-br from-red-50 to-pink-50 border-2 border-red-100 rounded-[2.5rem] p-8 shadow-sm">
        <div className="flex items-start mb-6 text-left">
           ...
           <div className="text-left">
              <p className="text-sm text-red-700 font-black uppercase tracking-tight mb-1 text-left">Kritik Duyuru Protokolü</p>
              <p className="text-xs text-red-600 font-bold leading-relaxed text-left">
                Yazdığınız mesaj tüm aktif kullanıcılara anlık iletilecektir. Bu işlem geri alınamaz.
              </p>
           </div>
        </div>
        <textarea
          ...
          rows="6"
          placeholder="Tüm kampüse ne söylemek istersin?"
          value={message}
        ...
        >
          {sending ? <Loader2 className="animate-spin" size={24} /> : <><Megaphone size={24} /> DUYURUYU YAYINLA</>}
        </button>
      </div>
    </div>
  );
